...
export const TaskForm: React.FC<Props> = ({
  ...
  const [isChild, setIsChild] = useState(!!initialData?.parentId);
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setIsChild(!!initialData.parentId);
    } else {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!isChild && formData.status === TaskStatus.COMPLETED && !formData.outcome) {
        ...
        <div className="flex justify-between items-center p-6 border-b sticky top-0 bg-white z-10">
          <div>
            <h2 className="text-xl font-bold text-slate-800">
              {initialData ? 'Editar Ação' : 'Nova Ação'}
            </h2>
            <p className="text-sm text-slate-500">Preencha os detalhes da atividade comercial</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {isChild ? (
              ...
              // MOTHER CATEGORY (Hidden or Fixed)
            ...
            {/* CONDITIONAL FIELDS FOR SPECIFIC CHILD ACTIONS */}
            ...
            <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* OUTCOME (Only for Mothers when Completed) */}

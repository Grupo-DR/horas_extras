...
export const calculateRecordFinancials = (record: ConstructionRecord, prices: ServicePrice[]) => {
  ...
  // Get unit and description from price data
  ...
  const descricao = serviceInfo?.descricao || record.item; // Fallback to item if not found
  ...
  return { valorRental, valorMobra, total: valorRental + valorMobra, status, unidade, descricao };
};

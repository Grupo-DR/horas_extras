...
export const DayDetailModal: React.FC<DayDetailModalProps> = ({ day, onClose }) => {
    ...
    const equipments = day.equipments || [];
            ...
            <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1">Equipamentos do Dia</p>
                    </div>
                    <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-full transition-colors">
                        <X className="w-5 h-5 text-slate-500" />
                    </button>
                ...
                <div className="space-y-2">
                    <p className="text-xs font-bold text-slate-500 uppercase mb-3">Equipamentos ({equipments.length})</p>
                    {equipments.length > 0 ? (
                        <div className="space-y-2">
                            {equipments.map((eq: any, i: number) => {
                                const eqDiff = eq.value - (eq.planned || 0);
                                return (
                                    <div key={i} className="bg-white border border-slate-200 p-4 rounded-xl hover:shadow-md transition-shadow">
                                        <div className="flex justify-between items-center">
                                            <span className="font-bold text-slate-900">{eq.frota}</span>
                                            <span className={`text-sm font-bold ${eqDiff >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                                                {eqDiff >= 0 ? '+' : ''}{formatCurrencyWithZero(eqDiff)}
                                            </span>
                                        </div>
                                        <div className="grid grid-cols-2 gap-4 mt-2">
                                            <div>
                                                <p className="text-[9px] text-slate-400 uppercase">Planejado</p>
                                                <p className="text-sm font-bold text-slate-600">{formatCurrencyWithZero(eq.planned || 0)}</p>
                                            </div>
                                            <div>
                                                <p className="text-[9px] text-slate-400 uppercase">Realizado</p>
                                                <p className="text-sm font-bold text-indigo-600">{formatCurrencyWithZero(eq.value)}</p>
                                            </div>
                                        </div>
                                    </div>
                                );

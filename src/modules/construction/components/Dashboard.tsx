...
const Dashboard: React.FC<DashboardProps> = ({ data, servicePrices, assignments }) => {
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      {stats.paretoCategory.length > 0 && (
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
          <h3 className="text-xs font-black text-slate-800 uppercase tracking-widest mb-6 flex items-center gap-2">
            <BarChart3 className="w-4 h-4 text-blue-500" /> Pareto por Categoria
          </h3>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={stats.paretoCategory} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#f1f5f9" />
                <XAxis type="number" tick={{ fontSize: 9 }} axisLine={false} tickFormatter={val => `R$ ${val / 1000}k`} />
                <YAxis dataKey="name" type="category" tick={{ fontSize: 9, fontWeight: 700 }} axisLine={false} width={100} />
                <Tooltip formatter={(v: any) => formatCurrencyWithZero(v)} />
                <Bar dataKey="value" name="Faturamento" radius={[0, 4, 4, 0]}>
                  {stats.paretoCategory.map((entry, index) => (
      ...
      {/* Ranking Improdutivo por Trecho */}
      {stats.idleTrechoTable.length > 0 && (
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
          <h3 className="text-xs font-black text-slate-800 uppercase tracking-widest mb-6 flex items-center gap-2">
            <MapPin className="w-4 h-4 text-red-500" /> Ranking Improdutivo por Trecho
          </h3>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-200">
                  <th className="text-left py-3 px-4 text-[10px] font-black text-slate-500 uppercase tracking-wider">Trecho</th>
                  <th className="text-right py-3 px-4 text-[10px] font-black text-slate-500 uppercase tracking-wider">Valor Improdutivo</th>
                  <th className="text-right py-3 px-4 text-[10px] font-black text-slate-500 uppercase tracking-wider">% do Total</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {stats.idleTrechoTable.map((row, i) => (
                  <tr key={i} className="hover:bg-slate-50 transition-colors">
                    <td className="py-3 px-4">
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-slate-900">{row.name}</span>
                        <div className="flex gap-2 mt-1">
                          {row.cities.slice(0, 3).map((city, j) => (
                            <span key={j} className="text-[9px] text-slate-500 bg-slate-100 px-2 py-0.5 rounded">
                              {city.name}
                            </span>
                          ))}
                        </div>
                    ...
                    <td className="py-3 px-4 text-right">
                      <span className="text-sm font-bold text-red-600">{formatCurrencyWithZero(row.value)}</span>
                    </td>
                    <td className="py-3 px-4 text-right">
                      <span className="text-sm font-bold text-slate-700">
                        {((row.value / stats.realImprodutivo) * 100).toFixed(1)}%
                      </span>
                    </td>
                  </tr>
                ))}

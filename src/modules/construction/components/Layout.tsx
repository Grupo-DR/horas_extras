      ...
      <aside className="w-64 bg-slate-900 text-white flex flex-col shrink-0">
        ...
        <nav className="flex-1 px-4 py-4 space-y-1">
          ...
          <button
            ...
            className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl transition-all duration-200 group relative overflow-hidden ${activeView === 'equipments'
              ? 'bg-gradient-to-r from-amber-500 to-amber-600 text-slate-900 shadow-sm'
              : 'text-slate-600 hover:bg-slate-50 hover:text-amber-600'
              }`}
          >
            <div className="flex items-center gap-3 relative z-10">
              <Truck className={`w-5 h-5 transition-transform duration-300 ${activeView === 'equipments' ? 'scale-110' : 'group-hover:scale-110'}`} />
              <span className="font-bold text-xs uppercase tracking-widest whitespace-nowrap overflow-hidden text-ellipsis">
                Equipamentos
              </span>
            </div>
          </button>
